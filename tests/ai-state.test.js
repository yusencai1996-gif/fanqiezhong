...
function fixture(initial = { schemaVersion: 10, settings: { workMinutes: 25, aiConfig: { apiKey: KEY, model: 'deepseek-chat' } }, sessions: [] }) {
  let disk = initial
  let writes = 0
  let failWrite = false
...
describe('AI state gateway', () => {
  ...
  it('returns only a masked config, including for short keys', () => {
    const f = fixture()
    expect(f.gateway.getPublicAiConfig()).toEqual({ hasApiKey: true, maskedApiKey: '••••5678', model: 'deepseek-chat' })
    const short = fixture({ settings: { aiConfig: { apiKey: 'abc', model: 'deepseek-chat' } } })
  ...
  it('v0.5.0:5/8/11 位密钥只显示圆点,12 位起才露尾四(掩码边界)', () => {
    const mask = key => fixture({ settings: { aiConfig: { apiKey: key, model: 'deepseek-chat' } } }).gateway.getPublicAiConfig().maskedApiKey
  ...
  it('v0.5.0:普通保存与配置更新都保留 aiConfig 额外字段(未来新增字段不被抹掉)', () => {
    const f = fixture({ schemaVersion: 10, settings: { aiConfig: { apiKey: KEY, model: 'deepseek-chat', futureFlag: true } }, sessions: [] })
    ...
    f.gateway.updateAiConfig({ model: 'deepseek-reasoner' })
    expect(f.disk.settings.aiConfig.futureFlag).toBe(true)
    expect(f.disk.settings.aiConfig.model).toBe('deepseek-reasoner')
    expect(f.disk.settings.aiConfig.apiKey).toBe(KEY)
  })
  ...
  it('ordinary save preserves the latest private key and model', () => {
    ...
    f.gateway.updateAiConfig({ model: 'deepseek-reasoner' })
    const stale = f.gateway.loadForRenderer()
    stale.settings.aiConfig = { apiKey: 'masked', model: 'deepseek-chat' }
    stale.sessions.push({ id: 'new' })
    expect(f.gateway.saveFromRenderer(stale)).toBe(true)
    expect(f.disk.settings.aiConfig).toEqual({ apiKey: KEY, model: 'deepseek-reasoner' })
  ...
  it('reports failed writes instead of success', () => {
    const f = fixture()
    f.failWrite = true
    expect(f.gateway.updateAiConfig({ model: 'deepseek-reasoner' }).error.code).toBe('CONFIG_SAVE_FAILED')
    expect(f.gateway.saveFromRenderer(f.gateway.loadForRenderer())).toBe(false)
    expect(f.disk.settings.aiConfig.model).toBe('deepseek-chat')
  })
  ...
  it('does not overwrite an unreadable state file', () => {
    ...
    expect(gateway.updateAiConfig({ model: 'deepseek-chat' }).error.code).toBe('CONFIG_SAVE_FAILED')

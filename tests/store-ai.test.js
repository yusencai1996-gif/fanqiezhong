...
describe('schema v10 AI configuration', () => {
  it('new store has a blank key and chat model', () => {
    ...
    expect(state.settings.aiConfig).toEqual({ apiKey: '', model: 'deepseek-chat' })
  })
  ...
  it('migrates v9 without losing other settings or business data', () => {
    ...
    expect(next.settings).toMatchObject({ workMinutes: 45, extra: true, aiConfig: { apiKey: '', model: 'deepseek-chat' } })
  ...
  it.each([undefined, null, 'bad', []])('tolerates invalid settings %s', settings => {
    expect(migrateState({ schemaVersion: 9, settings }).settings.aiConfig).toEqual({ apiKey: '', model: 'deepseek-chat' })
  })

  it.each([undefined, null, 'bad', []])('tolerates invalid AI config %s', aiConfig => {
    expect(migrateState({ schemaVersion: 9, settings: { aiConfig } }).settings.aiConfig).toEqual({ apiKey: '', model: 'deepseek-chat' })
  })

  it('preserves a valid key and reasoner choice', () => {
    const next = migrateState({ schemaVersion: 9, settings: { aiConfig: { apiKey: 'sk-test-12345678', model: 'deepseek-reasoner', extra: 1 } } })
    expect(next.settings.aiConfig).toEqual({ apiKey: 'sk-test-12345678', model: 'deepseek-reasoner', extra: 1 })
  })
  ...
  it('repairs invalid models even when the state already says v10', () => {
    expect(migrateState({ schemaVersion: 10, settings: { aiConfig: { apiKey: 'sk-test-12345678', model: 'unknown' } } }).settings.aiConfig)
      .toEqual({ apiKey: 'sk-test-12345678', model: 'deepseek-chat' })
  })
